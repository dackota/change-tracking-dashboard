...
func TestQueryChangesets_GroupsChangesByCommit(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(2*time.Hour)}, filter.FilterSpec{}, "", 100)
	if err != nil {
...
func TestQueryChangesets_AsOfBoundIsStrictlyLessThan(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(time.Hour)}, filter.FilterSpec{}, "", 100)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
...
func TestQueryChangesets_IncludeFilter(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(2*time.Hour)}, spec, "", 100)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
...
func TestQueryChangesets_ExcludeFilter_FacetAbsentSurvives(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(3*time.Hour)}, spec, "", 100)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
...
func TestQueryChangesets_MostRecentFirstOrdering(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(3*time.Hour)}, filter.FilterSpec{}, "", 100)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
...
func TestQueryChangesets_Pagination_WalksFullSetWithNoGapsOrOverlaps(t *testing.T) {
	...
	for pages := 0; pages < totalCommits+1; pages++ { // hard cap to avoid an infinite loop on a bug
		page, err := s.QueryChangesets(store.TimeWindow{AsOf: asOf}, filter.FilterSpec{}, cursor, pageSize)
		if err != nil {
...
func TestQueryChangesets_IncludeFilter_AppliesUnderSmallPageBound(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(10*time.Hour)}, spec, "", 2)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
...
func TestQueryChangesets_FilterAppliesPerChangeRowWithinACommit(t *testing.T) {
	...
	t.Run("include env=dev returns only the dev Change", func(t *testing.T) {
		...
		page, err := s.QueryChangesets(store.TimeWindow{AsOf: asOf}, spec, "", 100)
		if err != nil {
			t.Fatalf("QueryChangesets: %v", err)
		}
		if len(page.Changesets) != 1 {
...
func TestQueryChangesets_MultiChangeCommitAtPageBoundaryStaysWhole(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(4*time.Hour)}, filter.FilterSpec{}, "", pageSize)
	if err != nil {
		t.Fatalf("QueryChangesets: %v", err)
	}
	...
	nextPage, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(4*time.Hour)}, filter.FilterSpec{}, page.NextCursor, pageSize)
	if err != nil {
...
func TestQueryChangesets_NonPositiveLimitIsClampedToHardMax(t *testing.T) {
	...
	page, err := s.QueryChangesets(store.TimeWindow{AsOf: asOf}, filter.FilterSpec{}, "", 0)
	if err != nil {
	...
	pageOversized, err := s.QueryChangesets(store.TimeWindow{AsOf: asOf}, filter.FilterSpec{}, "", store.MaxChangesetPageSize*10)
	if err != nil {
...
func TestQueryChangesets_RejectsUnsafeFacetKey(t *testing.T) {
	...
	if _, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(time.Hour)}, includeSpec, "", 100); err == nil {
	...
	if _, err := s.QueryChangesets(store.TimeWindow{AsOf: csBase.Add(time.Hour)}, excludeSpec, "", 100); err == nil {
