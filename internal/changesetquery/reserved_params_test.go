...
// reservedParams (api_changesets.go) and store.reservedFacetNames
// (internal/store/store_filter.go). internal/store cannot import this package
// (it already imports store — importing back would create a cycle), so the
...
// the reservedParams package variable — never a re-typed string
...
// for every name in reservedParams, seed a Change whose Facets map
...
// reservedParams has (a one-sided addition), the corresponding
// subtest below fails.
func TestReservedChangesetsParams_ExcludedFromFacetOptions(t *testing.T) {
	t.Parallel()

	if len(reservedParams) == 0 {
		t.Fatal("reservedParams is empty — nothing to guard; the set may have moved or been renamed")
	}
	...
	for name := range reservedParams {
		t.Run(name, func(t *testing.T) {
			if vals, ok := opts[name]; ok {
				t.Errorf("FacetOptions returned reserved param %q (from changesetquery.reservedParams) as a facet key with values %v — store.reservedFacetNames has drifted out of sync", name, vals)
			}
		})
	}
